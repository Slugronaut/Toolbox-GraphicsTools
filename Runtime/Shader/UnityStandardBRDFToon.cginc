...
half4 BRDF_Unity_Toon (half3 diffColor, half3 specColor, half3 ramp, half atten, half oneMinusReflectivity, half smoothness,
    float3 normal, float3 viewDir,
    UnityLight light, UnityIndirect gi)
{
    float3 halfDir = Unity_SafeNormalize (float3(light.dir) + viewDir);
    half nl = smoothstep(0.0, 0.05, saturate(dot(normal, light.dir))); //apply toon shading with this
    //half nl = saturate(dot(normal, light.dir));
    float nh = saturate(dot(normal, halfDir));
